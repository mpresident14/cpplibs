#ifndef PREZ_INJECTOR_INJECTOR_HPP
#define PREZ_INJECTOR_INJECTOR_HPP
...
/**
 * @brief Associates type Value with type Key, such that injecting a Key will
 * call the corresponding injector for Value.
 *
 * @param <Key> The type to be bound. Must be decayed and the same or
 * convertible to Value.
 * @param <Value> The class to which Key is bound.
 * @param <Annotation> An extra identifier to differentiate between multiple
 * bindings of the same Key.
 *
 ...
 * @param val The value to which Key will be bound. Must be of type Value,
 * shared_ptr<Value>, or unique_ptr<Value>, where Value is convertible to Key
 *
 * @throw runtime_error if there is already a binding associated with Key
 *
...
/**
 * @brief Convenience wrapper for bindToObject<Key, Annotation, ValueHolder>
 * where Key is automatically inferred.
 *
 * @param val The value to which Key will be bound. Given val of type Value,
 * shared_ptr<Value>, or unique_ptr<Value>, Key is automatically inferred as
 ...
 *
 * @param <Key> The type to be bound. Must be a non-volatile non-reference.
 * @param supplier A function that takes in no arguments and returns Value,
 * shared_ptr<Value>, or unique_ptr<Value>, where Value is convertible Key. The
 * return type of supplier must be move-constructible.
 *
 * @throw runtime_error if there is already a binding associated with Key
 */
 ...
 *
 * @param KeyHolder The type of the object to inject.  Given KeyHolder of type
 * Key, shared_ptr<Key>, or unique_ptr<Key>, Key must be a non-volatile
 * non-reference.
 *
